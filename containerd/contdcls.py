...
class Ancestor:
     ...
     #def Ancestor(a,b):
        ...
        for line in t1.stdout.splitlines()[1:]:
           ...
           pa = subprocess.run("sudo ctr -n k8s.io image ls".format(r[0]), capture_output=True, shell=True, text=True, check=False)
           if (len(pa.stdout) > 1 ):
             print ("Image with ID:"+  r[0] + " is a CHILD")
             cl.append(r[0])
             cc = cc + 1
           else:
             print ("Image with ID:"+  r[0] + " is a PARENT")
             pl.append(r[0])
             pc = pc + 1 
        return(cl,pl,cc,pc)
...
class ImageArray(argparse.Action):
  ...
  #def ImageArray(a,string):
    ...
    for x in icl:
      c = "sudo ctr -n k8s.io image ls".format(x)
...
class Imageid:
  ...
  def __call__(self):
    ...
    t1 = subprocess.run("sudo ctr -n k8s.io containers list", capture_output=True, shell=True, text=True, check=True)
    l = []
    for line in t1.stdout.splitlines():
      r = re.findall(r'\S+',line)
      l.append(r[0])
...
class Containerid(argparse.Action):
  ...
  def __call__(self, parser, namespace, values, option_string=None):
  #def Containerid(a):
    t1 = subprocess.run("", capture_output=True, shell=True, text=True, check=True)
    l = []
    for line in t1.stdout.splitlines():
      r = re.findall(r'\S+',line)
      l.append(t)
...
class Action1(argparse.Action):
     ...
     def __call__(self, parser, namespace, values, option_string=None):
         ...
         print("ContainerID::::::::::::::::Image:::::::::::Runtime")
         for line in t1.stdout.splitlines()[1:]:
...
#     @staticmethod
   ...
   #  def Method1(self, dest, option_strings):
         g = "sudo ctr -n k8s.io containers list"
